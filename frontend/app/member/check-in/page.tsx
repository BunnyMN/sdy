...
export default function MemberCheckin() {
  ...
  return <div className="mx-auto max-w-lg space-y-5 rounded-2xl border border-line bg-surface p-6">
    <h1 className="text-2xl font-semibold">{t("events.checkin.title")}</h1>
    {error && <Banner tone="error" message={error} />}
    {done ? <><p role="status" className="text-accent">{t("events.checkin.success")}</p><Link href="/member/participation" className="inline-block py-3 text-accent">{t("events.participation")}</Link></>
      : !code ? <p className="text-muted">{t("events.checkin.missing")}</p>
        : !event && !error ? <Loading /> : event && <><h2 className="text-lg font-semibold">{event.title}</h2><p className="text-sm text-muted">{event.location}</p><p>{t("events.field.points")}: {event.points_value ?? 0}</p><button disabled={busy} onClick={() => void confirm()} className="min-h-12 w-full rounded-xl bg-accent px-4 font-semibold text-on-accent disabled:opacity-50">{t("events.checkin.confirm")}</button></>}
  </div>;
}
