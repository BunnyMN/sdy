...
export default function ParticipationPage() {
  ...
  return <div className="mx-auto max-w-3xl space-y-5">
    ...
    {points && <div className="rounded-2xl border border-line bg-surface p-5"><p className="text-sm text-muted">{t("events.points.total")}</p><p className="mt-2 text-4xl font-semibold text-accent">{points.total}</p></div>}
    {participation?.items.length === 0 && <p className="text-muted">{t("events.points.empty")}</p>}
    {participation?.items.map(item => <Link key={item.event_id} href={`/module/events/${item.event_id}`} className="flex items-center justify-between gap-3 rounded-xl border border-line bg-surface p-4"><span><strong className="block">{item.title}</strong><small className="text-muted">{t(`events.attendance.${item.status}`)}</small></span><strong className="text-accent">{item.points}</strong></Link>)}
    ...
    {points && points.entries.length > 0 && <section className="space-y-3"><h2 className="text-lg font-semibold">{t("events.points.history")}</h2>{points.entries.map(entry => <div key={entry.id} className="flex justify-between gap-3 rounded-xl border border-line p-4"><span>{entry.title}<small className="block text-muted">{new Date(entry.created_at).toLocaleString()}</small></span><strong>{entry.delta > 0 ? "+" : ""}{entry.delta}</strong></div>)}{points.has_more && <button disabled={busy} onClick={() => void more("points")} className="min-h-11 rounded-lg border border-line px-4">{t("events.more")}</button>}</section>}
  </div>;
}
